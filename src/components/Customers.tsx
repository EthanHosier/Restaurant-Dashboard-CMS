...
import {
  PendingReview,
  columns as reviewColumns,
} from "./table/pending-reviews-columns";
import {
  Customer,
  columns as allCustomerColumns,
} from "./table/customers-columns";
...
const Customers = ({
  ...
  function getRevData(
    ...
    const pendingReviews = bookings
      .filter(
        (e) =>
          !e.sentReview &&
          (e.locationId === selectedLocation || selectedLocation === "all") &&
          isMaxStayDurationExceeded(new Date(e.startDateTime), e.locationId)
      )
      .map((b) => {
        const {
          id,
  ...
  function getCustomerData(
    ...
    bookings
      .filter(
        (b) => b.locationId === selectedLocation || selectedLocation === "all"
      )
      .forEach((b, i) => {
        if (usersEmails.has(b.email)) {
          ...
          const { firstName, surname, mobileNumber, email, restaurantId } = b;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
      ...
      <div className="sm:col-span-1 lg:col-span-2">
        <DataCard
          ...
          value={pendingRevData.length}
          comment=""
          icon={<BadgeCheck width={iconWidth} />}
        />
      </div>{" "}
      {/* Component C */}
