    ...
    <>
      {src && (
        <Card className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 shadow w-2/3 max-h-3/4">
          <button
            className="absolute rounded-full shadow p-2 bg-white border -mt-2 -ml-2 z-10"
            onClick={() => setSrc("")}
            type="button"
          >
            <X size={18} />
          ...
          <div className="p-4">
            {cover ? (
              ...
              <Cropper ref={cropperRef} src={src} className="cropper" />
            )}
      ...
      <div className="w-48 h-48 border flex items-center justify-center">
        {localImageSrc != "DELETE" && (localImageSrc || websiteImageSrc) ? (
          <div className="relative">
            <button
              className="absolute rounded-full shadow p-2 bg-white border -mt-2 -ml-2"
              onClick={() => removeLocalImage(name, imageType, imageIndex)}
              type="button"
            >
              <X size={12} />
            </button>
            <img
              src={
                localImageSrc && localImageSrc != "DELETE"
                  ? localImageSrc
                  : websiteImageSrc
              }
              className={`w-full extra-images cursor-pointer ${
                cover ? "object-cover aspect-square" : "object-contain"
              } max-h-48`}
              alt={`${name} image`}
            />
          </div>
          ...
          <div className="relative w-full h-full">
            {websiteImageSrc && localImageSrc === "DELETE" && (
              <div className="w-48 flex justify-center mt-4">
                <Button
                  ...
                  variant={"secondary"}
                  onClick={() =>
                    handleRestoreImage(name, imageType, imageIndex)
                  }
                >
            ...
            <input
              ...
              onChange={(e) =>
                /*setLocalImage(e, imageType, imageIndex)*/ onUploadImage(e)
              }
              className="hidden"
              multiple={false}
            />
